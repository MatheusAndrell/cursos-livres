...
class MenuController extends Controller
{
    ...
    {
        $menu_items = MenuItems::where('menu', 1)->orderBy('sort')->get();
        $pages = Page::where('published', '=', 1)->get();
        $default_items = [];
        $default_menu_pages = [];
        return view('backend.menu-manager.index', compact('menu_items', 'pages', 'default_menu_pages', 'default_items'));
    }
    ...
    public function store(Request $request): \Illuminate\Http\RedirectResponse
    {
        ...
        if (isset($requestData['custom_pages'])) {
            ...
            MenuItems::where('id', $request->menu_item_id)->update($requestData);
        } else {
            $select_max_order = MenuItems::max('sort');
            $select_max_order = (int)$select_max_order;
            $requestData['sort'] = ++$select_max_order;
            MenuItems::create($requestData);
        }
        return redirect()->route('admin.menu-manager')->withFlashSuccess(trans('alerts.backend.general.created'));
    }

    public function delete($id): \Illuminate\Http\RedirectResponse
    {
        MenuItems::destroy($id);
        return redirect()->route('admin.menu-manager')->withFlashSuccess(trans('alerts.backend.general.deleted'));
    }
    ...
    public function update(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        foreach ($requestData['sortMenuItems'] as $sort_order => $menuItemId) {
            MenuItems::where('id', $menuItemId)->update(['sort' => $sort_order]);
        }
        return response()->json(['status' => true, 'message' => trans('alerts.backend.general.updated')]);
    }
