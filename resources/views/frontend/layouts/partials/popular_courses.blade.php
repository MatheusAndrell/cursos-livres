                ...
                @foreach($popular_courses as $item)
                    <div class="course-item-pic-text ">
                        <div class="course-pic relative-position mb25" @if($item->course_image != "")  style="background-image: url({{asset('storage/uploads/'.$item->course_image)}})" @endif>
                            ...
                            <div class="course-price text-center price-course">
                                @if($item->free == 1)
                        ...
                        <div class="course-item-text">
                            <div class="course-meta">
                                ...
                                @foreach($item->teachers as $teacher)
                                        <a class="author-name" href="#">{{$teacher->first_name}}</a></span>
                                @endforeach
                            ...
                            <div class="course-title mt10 headline pb45 relative-position">
                                <h3><a class="popular-courses" href="{{ route('courses.show', [$item->slug]) }}">{{$item->title}}</a>
                                    @if($item->trending == 1)
                                        <span
