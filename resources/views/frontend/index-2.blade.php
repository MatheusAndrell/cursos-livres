...
@section('content')
    ...
    @if ($sections->faq->status == 1)
        ...
        @include('frontend.layouts.partials.faq', [
            'classes' => 'faq-secound-home-version backgroud-style',
        ])
        <!-- End FAQ section
                ============================================= -->
    @endif
                        ...
                        @foreach ($teachers as $item)
                            <div class="teacher-img-text relative-position text-center">
                                <div class="teacher-img-social relative-position">
                                    <img height="200px" width="200px" src="{{ $item->picture }}"
                                        alt="{{ $item->full_name }}">
                                    <div class="blakish-overlay"></div>
                                    <div class="teacher-social-list ul-li">
                                        <ul>
                                            <li><a href="{{ 'mailto:' . $item->email }}"><i class="fa fa-envelope"></i></a>
                                            </li>
                                            <li><a href="{{ route('admin.messages', ['teacher_id' => $item->id]) }}"><i
                                            ...
                                            <div class="address-details ul-li-block">
                                                <ul>
                                                    @if ($contact_data['primary_address']['status'] == 1)
                                                        <li>
                                                            <span>@lang('labels.frontend.layouts.partials.primary'):
                                                            </span>{{ $contact_data['primary_address']['value'] }}
                                                        </li>
                                                    @endif

                                                    @if ($contact_data['secondary_address']['status'] == 1)
                                                        <li>
                                                            <span>@lang('labels.frontend.layouts.partials.second'):
                                                            </span>{{ $contact_data['secondary_address']['value'] }}
                                                        </li>
                                                    @endif
                                                </ul>
                                            </div>
                                            ...
                                            <div class="address-details ul-li-block">
                                                <ul>
                                                    @if ($contact_data['primary_phone']['status'] == 1)
                                                        <li>
                                                            <span>@lang('labels.frontend.layouts.partials.primary'):
                                                            </span>{{ $contact_data['primary_phone']['value'] }}
                                                        </li>
                                                    @endif
                                                    ...
                                                    @if ($contact_data['secondary_phone']['status'] == 1)
                                                        <li>
                                                            <span>@lang('labels.frontend.layouts.partials.second'):
                                                            </span>{{ $contact_data['secondary_phone']['value'] }}
                                                        </li>
                                                    @endif
                                                </ul>
                                            </div>
                                            ...
                                            <div class="address-details ul-li-block">
                                                <ul>
                                                    @if ($contact_data['primary_email']['status'] == 1)
                                                        <li>
                                                            <span>@lang('labels.frontend.layouts.partials.primary'):
                                                            </span>{{ $contact_data['primary_email']['value'] }}
                                                        </li>
                                                    @endif
                                                    ...
                                                    @if ($contact_data['secondary_email']['status'] == 1)
                                                        <li>
                                                            <span>@lang('labels.frontend.layouts.partials.second'):
                                                            </span>{{ $contact_data['secondary_email']['value'] }}
                                                        </li>
                                                    @endif
                                                </ul>
                                            </div>
                            ...
                            <div class="contact_secound_form text-white">
                                ...
                                <form class="contact_form" action="{{ route('contact.send') }}" method="POST"
                                    ...
                                    <div class="contact-info">
                                        <input
                                            class="name @if ($errors->has('name')) border-bottom border-danger @endif"
                                            name="name" type="text" placeholder="@lang('labels.frontend.contact.your_name')">
                                    </div>
                                    <div class="contact-info">
                                        <input
                                            class="email  @if ($errors->has('email')) border-bottom border-danger @endif"
                                            name="email" type="email" placeholder="@lang('labels.frontend.contact.your_email')">
                                    </div>
                                    <textarea name="message" class="@if ($errors->has('message')) border-bottom border-danger @endif"
                                        placeholder="@lang('labels.frontend.contact.message')"></textarea>
            ...
            <div class="footer_2 backgroud-style">
                <div class="container">
                    <div class="back-top text-center mb45">
                        <a class="scrollup" href="#"><img src={{ asset('assets/img/banner/bt.png') }}
                                alt=""></a>
                    </div>
                        ...
                        @if ($footer_data->newsletter_form->status == 1)
                            <div class="subs-form relative-position">
                                <form action="{{ route('subscribe') }}" method="post">
                                    @csrf
                                    <input class="email" name="subs_email" required type="email"
                                    ...
                                    <div class="col-md-4">
                                        <div class="copy-right-menu-item float-right ul-li">
                                            <ul>
                                                ...
                                                @endforeach
                                                <li><a
                                                        href="{{ route('frontend.certificates.getVerificationForm') }}">@lang('labels.frontend.layouts.partials.certificate_verification')</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
